
import { Link } from "react-router-dom";
import { useEffect, useState } from "react";

const NotFound = () => {
  const [earthRotating, setEarthRotating] = useState(false);

  return (
    <div className="bg-black min-h-screen text-white overflow-hidden">
      <div className="stars">
        <div className="custom-navbar flex justify-between items-center p-4">
          <div className="brand-logo">
            <Link to="/">
              <img 
                src="https://substackcdn.com/image/fetch/f_webp,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F5b4a1e03-a78a-4508-af5e-9cea2a7dd2d0_1280x1280.png" 
                alt="Logo" 
                width="80px"
                className="rounded-full"
              />
            </Link>
          </div>
        </div>
        
        <div className="central-body flex flex-col items-center justify-center py-20">
          <div className="text-5xl md:text-7xl font-bold mb-8 text-center">404</div>
          <div className="text-2xl md:text-4xl font-semibold mb-8 text-center">Lost in Space</div>
          <Link 
            to="/" 
            className="btn-3d px-6 py-3 bg-primary text-white rounded-full hover:bg-primary/80 transition-all"
          >
            GO BACK HOME
          </Link>
        </div>
        
        <div className="objects">
          <img 
            className="object_rocket" 
            src="data:image/svg+xml;base64," 
            width="70px"
          />
          
          <div className="earth-moon">
            <img 
              className={`object_earth ${earthRotating ? 'spin-earth-on-hover' : ''}`}
              src="data:image/svg+xml;base64," 
              width="100px"
              onMouseOver={() => setEarthRotating(true)}
            />
          </div>
          
          <div className="box_astronaut absolute top-[60%] right-[20%]">
            <img 
              className="object_astronaut" 
              src="data:image/svg+xml;base64," 
              width="140px"
            />
          </div>
        </div>
        
        <div className="glowing_stars absolute inset-0 overflow-hidden">
          <div className="star absolute w-1 h-1 bg-white rounded-full animate-[glow-star_2s_infinite_ease-in-out_alternate_1s] opacity-30" style={{ top: '80%', left: '25%' }}></div>
          <div className="star absolute w-1 h-1 bg-white rounded-full animate-[glow-star_2s_infinite_ease-in-out_alternate_3s] opacity-30" style={{ top: '20%', left: '40%' }}></div>
          <div className="star absolute w-1 h-1 bg-white rounded-full animate-[glow-star_2s_infinite_ease-in-out_alternate_5s] opacity-30" style={{ top: '25%', left: '25%' }}></div>
          <div className="star absolute w-1 h-1 bg-white rounded-full animate-[glow-star_2s_infinite_ease-in-out_alternate_7s] opacity-30" style={{ top: '75%', left: '80%' }}></div>
          <div className="star absolute w-1 h-1 bg-white rounded-full animate-[glow-star_2s_infinite_ease-in-out_alternate_9s] opacity-30" style={{ top: '90%', left: '50%' }}></div>
        </div>
      </div>
    </div>
  );
};

export default NotFound;
